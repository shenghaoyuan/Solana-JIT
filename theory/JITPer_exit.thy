...
proof-
  ...
  hence b5_1:"\<exists> v. Mem.loadv M64 m (Vlong ((reg SP) + (u64_of_memory_chunk M64))) = Some (Vlong v)" using match_state_def a6 match_stack_def by simp
  obtain v where b5_2:"Mem.loadv M64 m (Vlong ((reg SP) + (u64_of_memory_chunk M64))) = Some (Vlong v)" using b5_1 by blast
