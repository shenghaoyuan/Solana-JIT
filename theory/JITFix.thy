...
definition match_state2::"outcome \<Rightarrow> outcome \<Rightarrow> bool" where
  ...
  (case fxst of (Next xst xrs xm xss) \<Rightarrow>
    (case xxst of (Next xst1 xrs1 xm1 xss1) \<Rightarrow> 
     xst = xst1 \<and> xrs = xrs1 \<and> xm = xm1 \<and> xss = xss1 |
                   _ \<Rightarrow> False)|
                 _ \<Rightarrow> False)"*)
...
next
  ...
  have b1:"length prog' = length l_bin0" 
    ...
    subgoal for aa b
      apply(cases "x64_decode (nat (fst (l_pc ! nat (fst a)))) l_bin0",simp_all)
       subgoal for ab apply(cases ab,simp_all)
        subgoal for ac ba
          apply(cases ba,simp_all)
           apply(cases "x64_decode (nat (fst (l_pc ! nat (fst a))) + ac) l_bin0",simp_all)
...
(*\<and> x64_decode 0 l \<noteq> None \<and>*)
lemma jit_fix_not_change:
  "jitfix l_jump l_bin0 l_pc = Some prog \<Longrightarrow> 
  x64_decode xpc l_bin0 \<noteq> None \<Longrightarrow> 
  (\<not>(\<exists> num. x64_decode xpc l_bin0 = Some(num,Pret))) \<and> (\<not>(\<exists> num d. x64_decode xpc l_bin0 = Some(num, Pcall_i d))) \<and> (\<not> (\<exists> num src dst. x64_decode xpc l_bin0 = Some(num, Pcmpq_rr src dst))) \<Longrightarrow>
...
lemma one_steps_equiv_layer2:
  ...
  \<exists> num fst'. jit_fix_sem num prog xst = fst' \<and> match_state2 fst' (snd fxst')"
proof-
  ...
  next
    ...
    proof(cases "\<exists> num src dst. x64_decode xpc l_bin0 = Some(num, Pcmpq_rr src dst)")
      ...
      proof(cases "xrs1 (CR ZF) = 1")
        ...
        (case x64_decode (nat fix_begin_addr) l_bin0 of 
              Some(sz, Pcall_i _) \<Rightarrow> let offset = ((of_int target_begin_addr)::i32) - ((of_int fix_begin_addr+of_nat sz+1)::i32) in 
                                     let u8_list = x64_encode (Pcall_i (ucast offset)) in
                                     Some (x64_bin_update (length u8_list) l_bin0 (nat fix_begin_addr) u8_list) |
              Some(sz, Pcmpq_rr src dst) \<Rightarrow> let loc = nat fix_begin_addr+sz in 
              ...
              _ \<Rightarrow> None ))" using get_updated_l_bin_def by simp
                ...
                have "jit_fix_sem 2 prog' xst = x64_sem 2 prog' xst" using jit_fix_sem_def by simp
                hence e0_0:"jit_fix_sem 2 prog' xst =  (
               ...
               (* have "list_in_list (x64_encode(Pcmpq_rr src dst)) xpc l_bin0" using d2 d3_1 
                ...
                hence e1:"jit_fix_sem 2 prog' xst =  (
                  case x64_decode xpc prog' of
                ...
                have "jit_fix_sem 2 prog xst = x64_sem 2 prog xst" using jit_fix_sem_def by simp
                hence e0:"jit_fix_sem 2 prog xst =  (
                  case x64_decode xpc prog of
                  None \<Rightarrow> Stuck |
                  Some (sz, ins) \<Rightarrow>
                    x64_sem 1 prog (exec_instr ins sz xpc xrs xm xss)
                ...
                hence "\<exists> src dst sz. jit_fix_sem 2 prog xst = x64_sem 1 prog (exec_instr (Pcmpq_rr src dst) sz xpc xrs xm xss)"
                  using e0 e1 by force
                then obtain src dst sz where e2:"jit_fix_sem 2 prog xst = x64_sem 1 prog (exec_instr (Pcmpq_rr src dst) sz xpc xrs xm xss)" by auto
    ...
    next
      ...
      next
        ...
        have b1:"jit_fix_sem (snd (l_pc!(unat pc))) prog xst = x64_sem (snd (l_pc!(unat pc))) prog xst" 
          using jit_fix_sem_def by auto
        ...
        proof(cases "list_in_list l xpc l_bin0")
          ...
          hence "(jit_fix_sem (snd (l_pc!(unat pc))) prog xst) = (snd fxst')" 
          using b0 b1 list_in_list_prop4 a1 by (metis True a3 b2)  
        hence "\<exists> num fst'. num = (snd (l_pc!(unat pc))) \<and> jit_fix_sem num prog xst = fst' \<and> match_state2 fst' (snd fxst')"
...
proof(induct n arbitrary:l_bin0 l_pc l_jump pc xst fxst' xpc xrs xm xss xpc'' xrs'' xm'' xss'' prog lt)
  ...
  have a0:"jit_fix_sem 0 prog xst = xst"  by (simp add: jit_fix_sem_def) 
  have a1:"snd (flat_bpf_sem 0 (l_bin0,l_pc,l_jump) (pc,xst)) = xst" by simp

  have "\<exists> num fst'. num = 0 \<and> jit_fix_sem num prog xst = fst'" using a0 by blast

  hence "\<exists> num fst'. num = 0 \<and> jit_fix_sem num prog xst = fst' \<and> match_state2 fst' (snd fxst')"
  using a0 a1 apply(unfold match_state2_def jit_fix_sem_def,simp_all) 
  ...
  then show ?case by blast
    (*by (smt (verit, ccfv_threshold) flat_bpf_sem.simps(1) jit_fix_sem_def match_state2_def outcome.simps(4) snd_conv x64_sem.simps(1))*) 
next
  case (Suc n)
  ...
  hence "\<exists> num next_s. jit_fix_sem num prog xst = next_s \<and> match_state2 next_s (snd next_f)"
  ...
  then obtain num next_s where b1:"jit_fix_sem num prog xst = next_s \<and> match_state2 next_s (snd next_f)" by auto
  ...
  have "flat_bpf_sem n (l_bin0, l_pc, l_jump) (pc, xst) = fxst' \<Longrightarrow>
      ...
      snd fxst' = Next xpc'' xrs'' xm'' xss'' \<Longrightarrow> 
      jitfix l_jump l_bin0 l_pc = Some prog \<Longrightarrow> 
      \<exists>(num::nat) fst'::outcome. jit_fix_sem num prog xst = fst' \<and> match_state2 fst' (snd fxst')" using Suc by simp
  hence "\<exists>(num::nat) fst'::outcome. jit_fix_sem num prog (snd next_f) = fst' \<and> match_state2 fst' (snd fxst')" 
    using b2 b1 match_state2_def assm3 b0 assm2 b3
    by (metis Suc.hyps Suc.prems(5) Suc.prems(6) prod.collapse) 
    
  then show ?case
    by (metis Suc.prems(2) b1 jit_fix_sem_def match_state2_def x64_sem_add) 
qed
...
lemma n_steps_equiv:
  ...
  (*assm10:"jit_fix_sem n prog xst = fst'" and*)
  assm11:"jitper insns = Some lt"
  shows "\<exists> num fst'. jit_fix_sem num prog fxst = fst' \<and> match_state1 fst' (snd xxst')"
proof-
  ...
  have b2:"\<exists> num fst'. jit_fix_sem num prog fxst = fst' \<and> match_state2 fst' (snd fxst')" 
    using n_steps_equiv_layer2 assm7 assm9 b1 assm8 assm6 assm5 by blast
  then obtain num fst' where b3:"jit_fix_sem num prog fxst = fst' \<and> match_state2 fst' (snd fxst')" by auto
  ...
  have "\<exists> num fst'. jit_fix_sem num prog fxst = fst' \<and> match_state1 fst' (snd xxst')" using b2 b4 b3 by blast
