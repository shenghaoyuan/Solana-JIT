...
lemma l_pc_index_corr2:
  "find_target_pc_in_l_pc2 l_pc (int xpc) 0 = Some pc \<Longrightarrow> nat (fst (l_pc!(nat pc))) = xpc"
  sorry
...
proof-
  ...
  proof(cases "\<exists> num d. x64_decode xpc l_bin0 = Some(num, Pcall_i d)")
    ...
    have c0:"fxst' = (let v = find_target_pc_in_l_pc2 l_pc (int xpc) 0 in
                      (case v of None \<Rightarrow> Stuck | Some pc \<Rightarrow>
                      (case find_target_pc_in_l_pc l_jump pc of
                        None \<Rightarrow> Stuck |
                        Some npc \<Rightarrow>
                      exec_instr (Pcall_i (of_int(fst (l_pc!(unat npc))))) sz xpc xrs xm xss)))" using b1 c0_0 by simp
    
    hence "find_target_pc_in_l_pc2 l_pc (int xpc) 0 \<noteq> None" using a1
      using option.simps(4) by fastforce 
    hence "\<exists> pc. find_target_pc_in_l_pc2 l_pc (int xpc) 0 = Some pc" by simp
    then obtain pc where c1:"find_target_pc_in_l_pc2 l_pc (int xpc) 0 = Some pc" by auto
    ...
    have c3:"fxst' = exec_instr (Pcall_i (of_int(fst (l_pc!(unat npc))))) sz xpc xrs xm xss"
      using c0 c1 c2 by simp

    hence d0:"(pc,npc) \<in> set l_jump" using c1 c2 search_l_jump
    ...
    hence "jitfix [(pc,npc)] l_bin0 l_pc = (let res = get_updated_l_bin (pc,npc) l_bin0 l_pc in case res of None \<Rightarrow> None | 
                                                        Some l' \<Rightarrow> Some l')" by auto
    hence d1:"jitfix [(pc,npc)] l_bin0 l_pc = get_updated_l_bin (pc,npc) l_bin0 l_pc" 
      using a2 by(cases "get_updated_l_bin (pc,npc) l_bin0 l_pc",simp_all)
    let "?prog" = "jitfix [(pc,npc)] l_bin0 l_pc"
    have d2:"?prog = (let fix_begin_addr = fst (l_pc!(nat pc)); 
    ...
    have d2_0:"(fst (l_pc!(nat pc))) \<ge> 0" using a3 sorry
    have d2_1:"nat (fst (l_pc!(nat pc))) = xpc" using l_pc_index_corr2 c1 by blast     
    have d2_2:"(fst (l_pc!(nat pc))) = int xpc" using d2_1 d2_0 by presburger     
    have d2_3:"of_int (int xpc) = of_nat xpc" using d2_0 by simp
        ...
        (case x64_decode xpc l_bin0 of 
              ...
              Some(sz, Pcmpq_rr src dst) \<Rightarrow> let loc = xpc+sz in 
                    (case x64_decode loc l_bin0 of 
                            ...
                            _ \<Rightarrow> None ) |
              _ \<Rightarrow> None ))"
      using d2_1 d2_2 d2_3 d2 sorry 
    hence d3:"?prog = (let u8_list = x64_encode (Pcall_i (of_int ?target_begin_addr)) in
                                     Some (x64_bin_update (length u8_list) l_bin0 xpc u8_list))"
      using b1 by(cases "x64_decode xpc l_bin0",simp_all)
    
    let "?u8_list" = "(of_int ?target_begin_addr)"
    let "?len_u8_list" = "length(x64_encode (Pcall_i (of_int ?target_begin_addr)))"
    ...
    have d5:"x64_decode xpc someprog = Some (?len_u8_list,Pcall_i ?u8_list)" 
      using x64_bin_update_decode_incode_consistency d4_1 by blast
  ...
  next
    ...
    proof(cases "\<exists> num cond d. x64_decode xpc l_bin0 = Some(num, Pjcc cond d)")
      ...
      have c0:"fxst' = (let v = find_target_pc_in_l_pc2 l_pc (int xpc-3) 0 in
                      (case v of None \<Rightarrow> Stuck | Some pc \<Rightarrow>
                      (case find_target_pc_in_l_pc l_jump pc of
                        None \<Rightarrow> Stuck |
                        Some npc \<Rightarrow>
      ...
      have "\<exists> pc. find_target_pc_in_l_pc2 l_pc (int xpc-3) 0 = Some pc"
        using c0 a1 option.simps(4) by fastforce 
      then obtain pc where c1:"find_target_pc_in_l_pc2 l_pc (int xpc-3) 0 = Some pc" by auto
      have "\<exists> npc. find_target_pc_in_l_pc l_jump pc = Some npc"
        using c0 c1 a1 option.exhaust_sel by fastforce
      then obtain npc where c2:"find_target_pc_in_l_pc l_jump pc = Some npc" by auto
      have c3:"fxst' = (exec_instr (Pjcc cond (of_int (fst (l_pc!(unat npc)))-(of_nat (xpc+1)))) sz xpc xrs xm xss)"
        using c0 c2 c1 by force

      have d0:"(pc,npc) \<in> set l_jump" using c2 search_l_jump
        by (metis a4 distinct.simps(1) init_second_layer_def is_increase_list_empty l_jump_is_distinct list.simps(8)) 
      have "l_bin0 \<noteq> []" using a2
    ...
    have d2:"?prog = (let fix_begin_addr = fst (l_pc!(nat pc)); 
      ...
      using get_updated_l_bin_def d1 by auto
    have d2_0:"(fst (l_pc!(nat pc))) \<ge> 0" using a3 sorry
    have d2_1:"nat (fst (l_pc!(nat pc))) = xpc" using l_pc_index_corr2 c1 sorry
    have d2_2:"(fst (l_pc!(nat pc))) = int xpc" using d2_1 d2_0 by presburger     
    have d2_3:"of_int (int xpc) = of_nat xpc" using d2_0 by simp
    let "?target_begin_addr" = "fst(l_pc!(unat npc))"
    have "?prog = (
