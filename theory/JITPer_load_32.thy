...
proof-
  ...
  let "?c" = "chk"
  have b1:"construct_rex_to_u8 (?c = M32) (and (u8_of_ireg ?r1) 0b1000 \<noteq> 0) False (and (u8_of_ireg R11) 0b1000 \<noteq> 0) \<noteq> 0x40"
...
lemma load_m64_one_step_match_reg_1:
  assumes a0:"(SBPF_OK pc' rs' m' ss') = sbpf_step prog (SBPF_OK pc rs m ss)" and
    ...
    a5:"chk = M32"
  shows "\<exists> somev. loadv M32 xm (Vlong (scast off + xrs (IR (bpf_to_x64_reg src)))) = Some (Vlong somev)"
proof-
...
(* 4.1  match_reg *)
         ...
         apply(cases "loadv M32 xm (Vlong (scast off + xrs (IR (bpf_to_x64_reg src))))",simp_all)
        ...
        subgoal for a using a9 apply (cases a,simp_all) 
          ...
          subgoal for x5 
            apply(rule conjI)
            using load_M32_one_step_match_reg a0 a1 a2 a3 a4 a5 a6 a8 a9 apply auto[1]
(* 4.2  match_mem *)
            apply(rule conjI)
            using load_M32_one_step_match_mem a0 a1 a2 a3 a4 a5 a6 a8 a9 apply metis
(* 4.3  match_stack *)
            using load_M32_one_step_match_stack a0 a1 a2 a3 a4 a5 a6 a8 a9 apply auto
            using load_M32_one_step_match_stack2 a0 a1 a2 a3 a4 a5 a6 a8 a9 by (metis (no_types, lifting))
          done
        done
      done
    done
