...
lemma shiftq_arsh_one_step_match_reg:
  " (SBPF_OK pc' rs' m' ss') = sbpf_step prog (SBPF_OK pc rs m ss) \<Longrightarrow>
    ...
    (bpf_to_x64_reg dst) \<noteq> RSP \<Longrightarrow>
     match_reg rs'
     ((\<lambda>a::preg.
          if a = IR (bpf_to_x64_reg dst)
          then ucast
                (arsh64 (scast (xrs (IR (bpf_to_x64_reg dst)))) (unat (and (ucast (xrs (IR (bpf_to_x64_reg src)))) (63::32 word))))
          else if a = IR RCX then xrs (IR (bpf_to_x64_reg src))
               else if a = IR SP then xrs (IR SP) - u64_of_memory_chunk M64 else xrs a)
      (IR SP := xrs (IR SP), IR RCX := xrs (IR RCX)))"
  ...
  " (SBPF_OK pc' rs' m' ss') = sbpf_step prog (SBPF_OK pc rs m ss) \<Longrightarrow>
    ...
    match_stack
     ((\<lambda>a::preg.
      ...
      (IR SP := xrs (IR SP), IR RCX := xrs (IR RCX)))"
  by (simp add: match_state_def match_stack_def eval_alu_def eval_alu64_aux3_def eval_reg_def)


lemma encode_aux3:"(x64_encode(Ppushl_r x64Syntax.RCX)@x64_encode(Pmovq_rr (x64Syntax.RCX) (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst))@x64_encode(Ppopl x64Syntax.RCX))!0 \<notin> {0xc3, 0xe8}"
  subgoal apply(unfold per_jit_mul_reg64_def x64_encode_def)
   by(unfold construct_rex_to_u8_def bitfield_insert_u8_def Let_def u8_of_bool_def,simp_all) 
  done
...
(* 2. according to the code structure of JITPer, removing the first case statement *)
    ...
    subgoal
      using a5 a6 a8 aux5 per_jit_ins_def by fastforce
    subgoal
      apply (subgoal_tac "the (per_jit_shift_arsh_reg64 dst src) = (4, 0, x64_encode(Ppushl_r x64Syntax.RCX)@x64_encode(Pmovq_rr (x64Syntax.RCX) (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst))@x64_encode(Ppopl x64Syntax.RCX))")
       prefer 2
      subgoal using per_jit_shift_arsh_reg64_def a9 by(cases "(bpf_to_x64_reg dst)",simp_all)
      apply (subgoal_tac "(x64_encode(Ppushl_r x64Syntax.RCX)@x64_encode(Pmovq_rr (x64Syntax.RCX) (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst))@x64_encode(Ppopl x64Syntax.RCX)) !1 \<noteq> 0x39")
       prefer 2 subgoal apply(unfold per_jit_shift_arsh_reg64_def x64_encode_def) 
       (* using a9 apply(cases "bpf_to_x64_reg dst",simp_all) *)
        apply simp
          apply(unfold Let_def x64_encode_def construct_rex_to_u8_def bitfield_insert_u8_def construct_modsib_to_u8_def u8_of_bool_def)  
          apply(cases False,simp_all)
      ...
      apply(subgoal_tac "(x64_encode(Ppushl_r x64Syntax.RCX)@x64_encode(Pmovq_rr (x64Syntax.RCX) (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst))@x64_encode(Ppopl x64Syntax.RCX))!0 \<notin> {0xc3, 0xe8}")
      prefer 2 using encode_aux3 apply simp
      subgoal
        unfolding a3
        apply simp
        apply (erule subst)
...
(* 3.1.1 using consistency to get x64 assembly *)
        apply (subgoal_tac "x64_decode (0::nat)
            (x64_encode(Ppushl_r RCX)@x64_encode(Pmovq_rr RCX (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst))@x64_encode(Ppopl RCX))
            = Some (1, Ppushl_r RCX)")
         prefer 2
        subgoal
          apply (rule_tac l_bin = "x64_encode(Ppushl_r x64Syntax.RCX)" in x64_encode_decode_consistency)
...
(* using consistency to get x64 assembly *)
        apply (subgoal_tac "x64_decode (4::nat)
            (x64_encode(Ppushl_r RCX)@x64_encode(Pmovq_rr RCX (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst))@x64_encode(Ppopl RCX))
            = Some (3, Psarq_r (bpf_to_x64_reg dst))")
         prefer 2
(* this is necessary for later list_in_list_prop_aux2 *)
        apply (subgoal_tac "length (x64_encode(Ppushl_r RCX)@x64_encode(Pmovq_rr RCX (bpf_to_x64_reg src))) = 4")
         prefer 2 subgoal using x64_encode_def construct_rex_to_u8_def bitfield_insert_u8_def Let_def by simp
        subgoal
          apply (rule_tac l_bin = "x64_encode(Psarq_r (bpf_to_x64_reg dst))" in x64_encode_decode_consistency)
          subgoal using list_in_list_prop_aux2
            by (metis append_assoc)
          subgoal by simp
          subgoal using x64_encode_def
        ...
        apply simp
        apply (erule subst [of _ "Some (3::nat,Psarq_r (bpf_to_x64_reg dst))"])
        apply (simp add: exec_instr_def)
(* 3.4 *)
        apply (subgoal_tac "Suc 0 = (1::nat)") prefer 2 subgoal by simp
        apply (erule_tac subst [of _ 1])
...
(* this is necessary for later list_in_list_prop_aux2 *)
        apply (subgoal_tac "length ((x64_encode(Ppushl_r RCX)@x64_encode(Pmovq_rr RCX (bpf_to_x64_reg src))@
        x64_encode(Psarq_r (bpf_to_x64_reg dst)))) = 7")
         prefer 2 subgoal using x64_encode_def construct_rex_to_u8_def bitfield_insert_u8_def Let_def by simp
        subgoal
          apply (rule_tac l_bin = "x64_encode(Ppopl RCX)" in x64_encode_decode_consistency)
          subgoal using list_in_list_prop_aux2
...
(* 4. now we get exec_instr (one step of x64 add assembly), we prove the \<and>, first left, then right *)
        ...
        apply (rule conjI)
(* 4.1  match_reg *)
        subgoal using shiftq_arsh_one_step_match_reg a0 a1 a2 a3 a4 a6 a8 a9
          by simp

        apply (rule conjI)
(* 4.2  match_mem *)
        subgoal using shiftq_arsh_one_step_match_mem a0 a1 a2 a3 a4 a6 a8 a9 by simp
(* 4.3  match_stack *)
        subgoal using shiftq_arsh_one_step_match_stack a0 a1 a2 a3 a4 a6 a8 a9 apply simp
          using shiftq_arsh_one_step_match_stack2 a0 a1 a2 a3 a4 a6 a8 a9 by (metis (no_types, lifting)) 
        done
      done
    done
  done
