...
proof-
    ...
    have c1_0:"?l_bin = snd (snd (the (per_jit_jcc cond dst src (scast x))))" using a8 by auto
    let "?tcond" = "Cond_e"
    ...
    have c0:"?l_bin = snd (snd (the (per_jit_jcc cond dst src (scast x))))" using a8 by fastforce
    ...
    then have c3:"x64_prog!(unat pc) = the (per_jit_jcc cond dst src (scast x))" using a8 by simp
    ...
    have cn:"x64_sem num l xst = ?st \<and> match_state s' (pc',?st)" 
      using jcc_subgoal_rr_generic a0 a1 a2 c7 a4 a3 per_jit_jcc_def a8 c4 c1_1 d1 c2 c7 by force
    
    have "exec_instr xins1 (of_nat sz1) xpc xrs xm = Next (xpc+(of_nat sz1))(compare_longs (xrs (IR (bpf_to_x64_reg src))) (xrs (IR (bpf_to_x64_reg dst))) xrs) xm"
    ...
    hence e1:"xrs (IR (bpf_to_x64_reg dst)) = xrs (IR (bpf_to_x64_reg src)) \<longrightarrow>xrs' (CR ZF) = 1" by (smt (verit, del_insts) crbit.distinct(1) crbit.distinct(5) crbit.distinct(7)  fun_upd_apply preg.inject(2) x64Semantics.of_optbool_def)
    have e2:"xrs (IR (bpf_to_x64_reg dst)) \<noteq> xrs (IR (bpf_to_x64_reg src)) \<longrightarrow>xrs' (CR ZF) = 0" by (smt (verit, del_insts) crbit.distinct(1) crbit.distinct(5) crbit.distinct(7) e0 fun_upd_apply preg.inject(2) x64Semantics.of_optbool_def)
    have e3_0:"off = scast x + 1" using c_aux per_jit_jcc_def c3 c1_1 by simp
    have e3:"off\<noteq>0" sorry
    have "x64_sem1 1 x64_prog (pc,xst) = (pc', Next 0 xrs' xm')"
      using a8 e1 e2 cn a0 a1 a2 a3 a4 a5 a6 a7 x64_sem1_pc_aux1 c_aux match_state_eqiv e3 c7 x64_sem1_pc_aux2 by auto
