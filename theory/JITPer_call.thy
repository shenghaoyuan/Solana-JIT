...
lemma mem_is_not_changed_by_call:
  ...
  apply(split if_splits,simp_all)
  apply(split if_splits,simp_all)
  apply(unfold eval_call_reg_def Let_def)
  apply(unfold push_frame_def Let_def update_stack_def)
  apply(split if_splits, simp_all)
  apply(unfold eval_reg_def)
  apply(split if_splits,simp_all)
  done

lemma call_reg_subgoal_aux1:
  "s' = sbpf_step prog s \<Longrightarrow> s = (SBPF_OK pc rs m ss) \<Longrightarrow> s' = (SBPF_OK pc' rs' m' ss') \<Longrightarrow> 
  prog \<noteq> [] \<and> unat pc < length prog \<and> unat pc \<ge> 0 \<Longrightarrow> prog!(unat pc) = BPF_CALL_REG src imm \<Longrightarrow> 
  rs' BR10 = (stack_pointer ss)"
  apply(cases "prog!(unat pc)",simp_all)
  subgoal for x171 
    apply(cases "eval_call_reg src imm rs ss pc",simp_all)
    subgoal for a apply(cases a,simp_all )
      apply(unfold eval_call_reg_def push_frame_def Let_def,simp_all)
      apply(cases "update_stack [eval_reg BR6 rs, eval_reg BR7 rs, eval_reg BR8 rs, eval_reg BR9 rs] (eval_reg BR10 rs) (pc + (1::64 word)) ss",simp_all)
      subgoal for aa
        apply(split if_splits,simp_all) by auto
      done
...
lemma call_reg_subgoal_aux4:"s' = sbpf_step prog s \<Longrightarrow> s = (SBPF_OK pc rs m ss) \<Longrightarrow> s' = (SBPF_OK pc' rs' m' ss') \<Longrightarrow> 
  prog \<noteq> [] \<and> unat pc < length prog \<and> unat pc \<ge> 0 \<Longrightarrow> prog!(unat pc) = BPF_CALL_REG src imm \<Longrightarrow> 
  pc' = (eval_reg src rs - program_vm_addr)div (of_nat INSN_SIZE)"
 apply(cases "prog!(unat pc)",simp_all)
  subgoal for x171 
    apply(cases "eval_call_reg src imm rs ss pc",simp_all)
    subgoal for a apply(cases a,simp_all )
...
lemma call_reg_one_step:
  ...
  a5:"jitper prog = Some x64_prog" and                      
  a6:"prog \<noteq> [] \<and> unat pc < length prog \<and> unat pc \<ge> 0" and
  a8:"prog!(unat pc) = BPF_CALL_REG src imm"
shows "\<exists> xst'. x64_sem1 1 x64_prog (pc,xst) = (pc',xst') \<and> 
  match_state s' (pc',xst')"
proof-
  have b0:"\<forall> r. r\<noteq>BR10 \<longrightarrow> rs r = rs' r" using a0 a1 a2 a6 a8 call_reg_subgoal_aux2 by simp 
  ...
  have c4:"(l!0 \<noteq> 0xc3 \<and> l!1 = 0xff) \<or> l!0 = 0xff" using c_aux c3 c3_0 apply(unfold per_jit_call_reg_def x64_encode_def)
    apply(cases "Pcall_r (bpf_to_x64_reg src)",simp_all)
    subgoal for x16 apply(unfold Let_def) apply(split if_splits)
       apply auto[1]
      apply(unfold construct_rex_to_u8_def bitfield_insert_u8_def u8_of_bool_def Let_def,simp_all)
      apply(cases "and (u8_of_ireg x16) (8::8 word) \<noteq> (0::8 word) ",simp_all)
       apply auto[1]
      by auto
    done
  have c4_1:"l!0 \<noteq> 0xc3" using c4 by auto
  have c5:"?one_step = (let caller = save_x64_caller xrs; fp = save_x64_frame_pointer xrs;
                           xrs' = upate_x64_stack_pointer xrs (stack_pointer xss) in
            let ss' = update_stack caller fp (pc+1) xss in
              (case ss' of None \<Rightarrow> (pc, Stuck) | 
              Some ra \<Rightarrow> ((pc - program_vm_addr) div (of_nat INSN_SIZE), Next xpc xrs' xm ra)))" using c4 c2_1 one_step_def a3 c_aux c4_1
    by (smt (z3) One_nat_def case_prod_conv fst_conv option.case_eq_if outcome.simps(4) snd_conv x64_sem1.simps(1) x64_sem1.simps(2))  

  have d1_1:"xss = ss" using match_state_def a0 a1 a2 a3 a4 by auto
  have d2_0:"call_depth xss +1 \<noteq>  max_call_depth" using c5 update_stack_def call_reg_subgoal_aux3 a0 a1 a2 a6 d1_1 using a8 by blast
  have d2_1:"?st \<noteq> Stuck" using d2_0 c5 by (simp add: update_stack_def) 
  ...
  have "fst ?one_step = (pc - program_vm_addr) div (of_nat INSN_SIZE)" using c5 by (metis (no_types, lifting) d2_1 option.case_eq_if split_pairs)
  hence "fst ?one_step = pc'" sorry
    thus ?thesis using d4 by (metis split_pairs)
  qed*)
