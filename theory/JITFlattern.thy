 ...
 have "\<exists> xpc1 xm1. fxst = Next xpc1 xrs xm1 xss \<and> JITFlattern_def.match_mem xm1 xm" using a3 a5 
  ...
  let old_xpc = fst (l_pc!(unat pc)) in 
        ...
        Some(sz, Pcmpq_rr src dst) \<Rightarrow> \<comment>\<open> TODO: if the first byte is the opcode of cmp? \<close>
          ...
          (case x64_decode (xpc1+sz) l_bin of Some (sz2,Pjcc cond _) \<Rightarrow>
          (if num = 1 then case x64_sem num l_bin (Next xpc1 xrs xm1 xss) of
  ...
  next
    ...
    next
      ...
      proof(cases "(\<exists> num src dst. x64_decode 0 ?l = Some(num, Pcmpq_rr src dst))")
        ...
        case True
        then obtain num src dst where d0_0:"x64_decode 0 ?l = Some(num, Pcmpq_rr src dst)" by auto
        hence d0:"xxst' = ((case x64_decode num ?l of Some(sz, Pjcc cond d ) \<Rightarrow> 
          let xst_temp = Next 0 xrs xm xss; xst' = x64_sem 1 ?l xst_temp in
          (case xst' of
          Next xpc' rs' m' ss'\<Rightarrow>
            if eval_testcond cond rs' then (?off+pc, xst')
          ...
          Stuck \<Rightarrow> (pc, Stuck))|
        _ \<Rightarrow> (pc,Stuck)))" using True b1 b0 b5 apply(cases "x64_decode 0 ?l",simp_all)
          apply(cases "lt ! unat pc",simp_all)
          subgoal for a b c
            apply(cases "x64_decode num c ",simp_all)
            subgoal for aa apply(cases a,simp_all)
               apply(cases aa,simp_all)
              subgoal for ab ba apply(cases ba,simp_all)
                subgoal for x131 x132 apply(unfold Let_def)
                  apply(cases "exec_instr (Pcmpq_rr src dst) num (0::nat) xrs xm xss",simp_all)             
              done
            done
          subgoal for nat apply(cases aa,simp_all) 
            subgoal for ab ba apply(cases ba,simp_all)
              subgoal for x131 x132 apply(unfold Let_def)
                apply(cases "exec_instr (Pcmpq_rr src dst) num (0::nat) xrs xm xss",simp_all)       
                done
      ...
      have d0_1:"\<exists> sz2 cond d. x64_decode num ?l = Some (sz2,Pjcc cond d)" 
      proof(rule ccontr)
        assume assm:"\<not>(\<exists> sz2 cond d. x64_decode num ?l = Some (sz2,Pjcc cond d))"
        then obtain sz2 cond d where "\<not>(x64_decode num ?l = Some (sz2,Pjcc cond d))" by auto
        hence "x64_decode num ?l \<noteq> Some (sz2,Pjcc cond d)" by auto
        hence "xxst' = (pc,Stuck)" using d0 using assm apply(cases "x64_decode num ?l",simp_all)
          subgoal for a apply(cases a,simp_all)
            subgoal for aa b apply(cases b,simp_all)
              done
            done
          done
        thus "False" using a6 by simp
      qed
      then obtain sz2 cond d where d0_2:"x64_decode num ?l = Some (sz2,Pjcc cond d)" by auto
      hence d0_3:"xxst' = (let xst_temp = Next 0 xrs xm xss; xst' = x64_sem 1 ?l xst_temp in
          (case xst' of
          Next xpc' rs' m' ss'\<Rightarrow>
            if eval_testcond cond rs' then (?off+pc, xst')
            else (pc+1, xst') |
          Stuck \<Rightarrow> (pc, Stuck)))" using d0_2 d0 by(cases "x64_decode num ?l",simp_all)
      ...
      have d2_5_1:"xxst1 \<noteq> Stuck" 
       proof(rule ccontr)  
         assume assm:"\<not>(xxst1 \<noteq> Stuck)" 
         hence "xxst' = (pc,Stuck)" using a6 d0_3 d2_1 by(unfold Let_def,simp_all)
         thus "False" using a6 by simp
       qed
       hence d0_4:"xxst' = (case xxst1 of
          Next xpc' rs' m' ss'\<Rightarrow>
            if eval_testcond cond rs' then (?off+pc, xxst1)
            else (pc+1, xxst1))" using d2_5_1 d2_1 d0_3
         by (metis (no_types, lifting) outcome.exhaust outcome.simps(4)) 
       have d2_5:"x64_decode 0 ?l \<noteq> None" using d2_1 d2_5_1 by(cases "x64_decode 0 ?l",simp_all)
       ...
       have d1_1:"\<exists> sz src dst. x64_decode xpc1 l_bin = Some(sz, Pcmpq_rr src dst)" 
        ...
        hence d1:"fxst' = (case x64_decode (xpc1+sz) l_bin of Some (sz2,Pjcc cond _) \<Rightarrow>
          (if (snd (l_pc!(unat pc))) = 1 then case x64_sem (snd (l_pc!(unat pc))) l_bin (Next xpc1 xrs xm1 xss) of
          Stuck \<Rightarrow> (pc, Stuck) | \<comment>\<open> if one step error, stop, it should be impossible \<close>
          Next xpc1 rs1 m1 ss1 \<Rightarrow> (
          case find_target_pc_in_l_pc l_jump (unat pc) of
                      None \<Rightarrow> (pc, Stuck) |
            ...
            _ \<Rightarrow> (pc, Stuck))" using True a0 c2 a7 a9 c1 apply(split if_splits,simp_all) by(unfold Let_def,simp_all)
        have d1_3:"x64_decode (xpc1+sz) l_bin = Some (sz2,Pjcc cond d)"
          by (metis add.right_neutral c3_0 d0_0 d0_2 d1_0 list_in_list_x64_decode old.prod.inject option.inject)
       have d4:"(snd (l_pc!(unat pc))) = ?num"
         by (metis (mono_tags, lifting) a1 a7 b0 flattern_num)     
       have b7:"?num = 1" by (metis a10 a7 b0 d0_0 jit_prog_prop4)    
       hence d1_4:"fxst' = ((case x64_sem (snd (l_pc!(unat pc))) l_bin (Next xpc1 xrs xm1 xss) of
          ...
          case find_target_pc_in_l_pc l_jump (unat pc) of
            ...
            if eval_testcond cond rs1 then \<comment>\<open> must JUMP \<close>
             (npc, (Next (fst (l_pc!(unat npc))) rs1 m1 ss1)) \<comment>\<open> go to the target address in the jited x64 binary \<close>
              ...
              (pc+1, Next (xpc1+sz2) rs1 m1 ss1)
          )))" using d1 d1_3 d4 by(cases "x64_decode (xpc1+sz) l_bin",simp_all)
        ...
        hence d4_2:"fxst' = (case find_target_pc_in_l_pc l_jump (unat pc) of
                      None \<Rightarrow> (pc, Stuck) |
                      Some npc \<Rightarrow>
              if eval_testcond cond xrs1 then  \<comment>\<open> must JUMP \<close>
        ...
        by (metis (mono_tags, lifting) Abs_fnat_hom_0 add_0 list.size(3))        
          ...
          proof(cases "eval_testcond cond xrs1")
            ...
            next
              case False
              have d5_0:"\<not> (eval_testcond cond xrs1)" using False by simp
              have d6_1:"xxst' = (pc+1, xxst1)" using d0_4 d4_1 False by force
